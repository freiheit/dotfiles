...
# ── LOCAL EDIT (freiheit, 2026-08-30): statusline section toggles ─────────────
# 1 = show, 0 = hide. `/LifeOS update` re-overlays this file and reverts these,
# so re-apply after an upgrade (see the install-deviations memory).
SL_SHOW_EFFORT_SCALE=0   # the LOW…ULTRA scale strip (live effort moved onto the model line)
SL_SHOW_VERSIONS=0       # LifeOS + ALGO version line (model line still renders)
SL_SHOW_CONTEXT=0        # CONTEXT bar, startup-file breakdown, STARTUP LOAD total
SL_SHOW_ACCOUNT_EMAIL=0  # logged-in account email
SL_SHOW_AGENT_ROSTER=0   # HAIKU…FABLE │ MAX FORGE roster (live in-flight agent lines stay)

# ── LOCAL EDIT (freiheit, 2026-09-06): LifeOS-context gating + flat layout ────
# LifeOS header, STATE, 🧠 memory, and quote lines render ONLY when the session
# cwd is under one of these prefixes OR a `.lifeos` marker file exists in cwd or
# any parent. MODEL/EFFORT + usage always render, merged onto one line. All
# ┄/─/· separator rules removed. `/LifeOS update` reverts this — re-apply.
SL_LIFEOS_DIRS=(
    "$HOME/Documents/freibrain"
    "$HOME/.claude"
    "$HOME/Code/personal-brief"
    "$HOME/Code/web/freiheit-wtf"   # ~/Code/freiheit-wtf symlinks here; realpath folds both
)
...
# every install without an explicit timezone configured (a real bug freiheit hit).
...
# LOCAL EDIT (freiheit, 2026-09-06): is this a LifeOS context? Dir-prefix list
# (fast path) plus `.lifeos` marker-file walk (no-edit escape hatch). realpath -m
# normalizes /home ↔ /var/home symlinks on this ostree host.
SL_IN_LIFEOS=0
_sl_cwd=$(realpath -m "$current_dir" 2>/dev/null || echo "$current_dir")
for _sl_d in "${SL_LIFEOS_DIRS[@]}"; do
    _sl_dr=$(realpath -m "$_sl_d" 2>/dev/null || echo "$_sl_d")
    case "$_sl_cwd/" in "$_sl_dr"/*) SL_IN_LIFEOS=1; break ;; esac
done
if [ "$SL_IN_LIFEOS" = 0 ]; then
    _sl_p="$_sl_cwd"
    while [ -n "$_sl_p" ] && [ "$_sl_p" != "/" ]; do
        [ -f "$_sl_p/.lifeos" ] && { SL_IN_LIFEOS=1; break; }
        _sl_p="${_sl_p%/*}"
    done
fi
...
# the leading edge changes color as it travels (freiheit, 2026-07-18). Unfilled
...
render_usage_meter() {
    ...
    # LOCAL EDIT (freiheit, 2026-09-06): cool→hot ramp, blue at the safe end.
    local bg_rest='\033[48;2;15;30;58m'       # dim navy — remaining budget
    local fg_rest='\033[22;38;2;130;160;200m' # soft blue-gray text (22 = drop bold bleed)
    # Whole fill is ONE color, picked from the meter's overall pct (not a
    # per-cell position gradient — that painted every bar blue-then-cyan).
    local bg_full fg_full
    if   [ "$pct_i" -gt 90 ]; then bg_full='\033[48;2;239;68;68m'   # bright red
    elif [ "$pct_i" -gt 75 ]; then bg_full='\033[48;2;217;72;35m'   # orange-red
    elif [ "$pct_i" -gt 62 ]; then bg_full='\033[48;2;194;98;30m'   # orange
    elif [ "$pct_i" -gt 50 ]; then bg_full='\033[48;2;176;128;16m'  # amber
    elif [ "$pct_i" -gt 35 ]; then bg_full='\033[48;2;22;140;74m'   # green
    elif [ "$pct_i" -gt 20 ]; then bg_full='\033[48;2;13;140;168m'  # cyan
    else bg_full='\033[48;2;37;99;235m'                             # blue
    fi
    fg_full=$fg_fill
    # 90%+: bold white on bright red so the alarm pops
    [ "$pct_i" -gt 90 ] && fg_full='\033[1;38;2;255;255;255m'
    local out="" cur="" bg fg i
    for ((i=1; i<=w; i++)); do
        if [ "$i" -le "$filled" ]; then
            bg=$bg_full
            fg=$fg_full
...
# Render context bar — SESSION-MIX fill (freiheit, 2026-07-28): the filled
...
# LOCAL EDIT (freiheit, 2026-09-06): LifeOS-context only.
if [ "$SL_IN_LIFEOS" = 1 ]; then
_hdr_loc=""
...
# LOCAL EDIT (freiheit, 2026-08-30): columns chosen by the principal. RHYTHMS
# replaces CREATIVITY here; both remain scored on disk and visible in Pulse.
# The lookup below is generic, so changing these two arrays is now the whole
# edit. `/LifeOS update` re-overlays this file and reverts it — re-apply after
# an upgrade (see the install-deviations memory).
_dims=(health rhythms relationships finances freedom)
_labels=(HEALTH RHYTHMS RELS FIN FREEDOM)
...
if [ -f "$_LIFEOS_STATE_JSON" ]; then
    # One jq call for whatever _dims holds. "finances" falls back to the legacy
    # .dimensions.money key. Absent values come back as the literal N/A rather
    # than "", so consecutive tabs can't collapse and misalign the columns.
    IFS=$'\t' read -r -a _state_vals <<< "$(
        # --args must come AFTER the input file, or jq swallows the filename as
        # a positional and reads an empty stdin instead.
        jq -r '[ $ARGS.positional[] as $d
          | (.dimensions[$d].pct
             // (if $d == "finances" then .dimensions.money.pct else null end)
             // "N/A" | tostring) ] | @tsv' "$_LIFEOS_STATE_JSON" --args "${_dims[@]}" 2>/dev/null
    )"
    for _i in "${!_dims[@]}"; do
        _v="${_state_vals[$_i]:-N/A}"
        [ "$_v" != "N/A" ] && [ "$_v" != "null" ] && _pcts[$_i]="${_v%%.*}"
    done
fi
...
# Dotted divider between STATE and MODE. Belongs to the effort strip; with that
# strip hidden it would sit directly above a solid rule, so it goes with it.
[ "${SL_SHOW_EFFORT_SCALE:-1}" = 1 ] && printf "${SLATE_600}%s${RESET}\n" "$SEP_DOT"
...
[ "${SL_SHOW_EFFORT_SCALE:-1}" = 1 ] && printf "%b\n" "$_pm_line"
...
# LOCAL EDIT (freiheit, 2026-09-06): LifeOS-context only.
if [ "$MODE" = "normal" ] && [ "$SL_IN_LIFEOS" = 1 ] && [ -f "$_review_state_file" ]; then
...
# LOCAL EDIT (freiheit, 2026-09-06): separator rule removed (flat layout).
...
# LOCAL EDIT (freiheit, 2026-08-30): versions hidden by default, and the live
# effort level rides on this same line instead of the LOW…ULTRA scale strip.
if [ "${SL_SHOW_VERSIONS:-1}" = 1 ]; then
    ...
    # LOCAL EDIT (freiheit, 2026-09-06): don't print here — stash the segment
    # and merge it onto the usage line below (one line: MODEL │ EFFORT │ 5H/WK).
    # Model name in bright sky-cyan, effort in its heat color (tinted labels).
    _model_seg_bare="$(printf '\033[38;2;56;189;248m')${_model_display}${RESET} ${SLATE_600}│${RESET} $(_pm_level_c "$_pai_level")${_pai_level}${RESET}"
    _model_seg="${_model_seg_bare} ${SLATE_600}│${RESET} "
fi
...
if [ "$MODE" = "normal" ]; then
    ...
    [ "${SL_SHOW_AGENT_ROSTER:-1}" = 1 ] && printf "%b\n" "$_ar_line"
fi
...
# The separator belongs to the context block; hiding the block hides its rule too.
[ "${SL_SHOW_CONTEXT:-1}" = 1 ] && sep
...
if [ "${usage_state:-absent}" = "api" ]; then
    ...
    # LOCAL EDIT (freiheit, 2026-09-06): model+effort merged on; rule removed.
    printf "%b📊 ${USAGE_PRIMARY}API${RESET} ${USAGE_LABEL}SESSION${RESET} ${SLATE_300}\$${_api_cost}${RESET}\n" "${_model_seg:-}"
    ...
    # empty cells read as missing data (freiheit, 2026-07-18).
    scoped_fmt=""
    ...
    # LOCAL EDIT (freiheit, 2026-09-06): model+effort merged on; rule removed.
    printf '%b' "${_model_seg:-}${_5h_label_color}5H${RESET} ${_bar_5h} ${_7d_label_color}WK${RESET} ${_bar_7d}${scoped_fmt} ${_billing_fmt}"
    [ -n "$stale_suffix" ] && printf '%b' "$stale_suffix"
    printf "\n"
else
    # LOCAL EDIT (freiheit, 2026-09-06): no usage data — MODEL/EFFORT still shows.
    [ -n "${_model_seg_bare:-}" ] && printf '%b\n' "$_model_seg_bare"
fi
...
if [ "$MODE" = "normal" ]; then
    ...
    if [ "${SL_SHOW_ACCOUNT_EMAIL:-1}" = 1 ]; then
...
# LOCAL EDIT (freiheit, 2026-09-06): LifeOS-context only.
if [ "$MODE" = "normal" ] && [ "$SL_IN_LIFEOS" = 1 ] && [ -f "$QUOTES_FILE" ]; then
